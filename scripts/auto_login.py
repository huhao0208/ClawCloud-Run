...
class Telegram:
    ...
    def __init__(self):
        # 读取并去除两端空格，避免因 secret 中包含换行/空格导致无效
        self.token = (os.environ.get('TG_BOT_TOKEN') or '').strip()
        self.chat_id = (os.environ.get('TG_CHAT_ID') or '').strip()
        self.ok = bool(self.token and self.chat_id)

        # 调试输出（帮助排查未发送的原因）
        if self.ok:
            print(f"✅ Telegram 已启用，chat_id={self.chat_id}")
        else:
            print("⚠️ Telegram 未启用（TG_BOT_TOKEN 或 TG_CHAT_ID 缺失或为空）")
            print(f" - TG_BOT_TOKEN: {'存在' if bool(self.token) else '缺失'}")
            print(f" - TG_CHAT_ID: {'存在' if bool(self.chat_id) else '缺失'}")
    
    def send(self, msg):
        if not self.ok:
            # 明确打印跳过原因，方便 CI / 本地日志排查
            print(f"[跳过 Telegram send] Telegram 未启用，消息未发送。摘要: {msg[:80]!s}")
            return
        try:
            r = requests.post(
            ...
            # 打印请求返回状态，便于诊断 401/403/429 等问题
            if r.status_code != 200:
                print(f"❌ Telegram send 失败: status={r.status_code}, body={r.text}")
            else:
                print("✅ Telegram send 成功")
        except Exception as e:
            print(f"❌ Telegram send 异常: {e}")
    
    def photo(self, path, caption=""):
        if not self.ok:
            print(f"[跳过 Telegram photo] Telegram 未启用，图片未发送: {path}")
            return
        if not os.path.exists(path):
            print(f"⚠️ Telegram photo: 文件不存在: {path}")
            return
        try:
            ...
            if r.status_code != 200:
                print(f"❌ Telegram photo 失败: status={r.status_code}, body={r.text}")
            else:
                print(f"✅ Telegram photo 已发送: {path}")
        except Exception as e:
            print(f"❌ Telegram photo 异常: {e}")
    
    def flush_updates(self):
        """刷新 offset 到最新，避免读到旧消息"""
        if not self.ok:
            print("[flush_updates] Telegram 未启用，返回 offset=0")
            return 0
        try:
            r = requests.get(
            ...
            if data.get("ok") and data.get("result"):
                ...
                print(f"[flush_updates] 已刷新 offset -> {offset} (共 {len(data['result'])} 条更新)")
                return offset
            print("[flush_updates] 无更新或请求未 ok")
        except Exception as e:
            print(f"❌ flush_updates 异常: {e}")
        return 0
    ...
    def wait_code(self, timeout=120):
        ...
        """
        if not self.ok:
            print("[wait_code] Telegram 未启用，无法等待验证码")
            return None
        ...
        while time.time() < deadline:
            try:
                ...
                data = r.json()
                if not data.get("ok"):
                    print("[wait_code] getUpdates 未 ok，稍后重试")
                    time.sleep(2)
                    continue
                ...
                for upd in data.get("result", []):
                    ...
                    if str(chat.get("id")) != str(self.chat_id):
                        print(f"[wait_code] 跳过来自 chat_id={chat.get('id')} 的消息")
                        continue
                    
                    text = (msg.get("text") or "").strip()
                    m = pattern.match(text)
                    if m:
                        print("[wait_code] 收到验证码消息（已返回）")
                        return m.group(1)
            
            except Exception as e:
                print(f"❌ wait_code 异常: {e}")
            
            time.sleep(2)
        
        print("[wait_code] 等待验证码超时")
        return None
